"""Run an external JSON agent through explicit runner phases.

The configured command receives fixed cases in ``ORBIT_CYCLE_INPUT`` and must
emit one JSON object for each action. Orbit preserves those responses as
...
def preflight(ctx):
    """Capture the agent's one-time readiness result."""
...
def prepare(ctx):
    """Prepare the agent for the current bounded iteration."""
...
def execute(ctx):
    """Run the agent once and preserve its structured output."""
...
def verify(ctx):
    """Collect the agent's evidence payload for supervision."""
    ctx.emit_result(
...
def after_each(ctx):
    """Close one external-agent iteration."""
...
def after_all(ctx):
    """Finalize the external-agent evaluation."""
